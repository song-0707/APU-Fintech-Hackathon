        ...
        <div className="space-y-2">
          ...
          <div className="grid gap-2 lg:grid-cols-2">
            {minuteSummaries.map((summary) => (
              <article key={summary.id} className="rounded-xl border border-violet-100 dark:border-violet-900/50 bg-violet-50/40 dark:bg-violet-950/30 p-3 text-xs text-slate-700 dark:text-slate-300">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="font-bold text-violet-700 dark:text-violet-300">{summary.label}</span>
                  <span className="rounded-full bg-violet-100 dark:bg-violet-900/60 px-2 py-0.5 text-[10px] font-semibold text-violet-700 dark:text-violet-300">Live</span>
                </div>
                <p>{summary.summary}</p>
                {summary.decisions.length > 0 && (
                  <div className="mt-2">
                    <div className="font-bold text-slate-600 dark:text-slate-400">Decisions</div>
                ...
                {summary.action_items.length > 0 && (
                  <div className="mt-2">
                    <div className="flex items-center gap-1 font-bold text-slate-600 dark:text-slate-400">
                      <ListChecks className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" /> Action items
                    </div>
                    <ul className="mt-1 list-disc space-y-0.5 pl-4">
                ...
                {summary.risks.length > 0 && (
                  <div className="mt-2">
                    <div className="flex items-center gap-1 font-bold text-slate-600 dark:text-slate-400">
                      <AlertTriangle className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" /> Risks
                    </div>
                    <ul className="mt-1 list-disc space-y-0.5 pl-4">
